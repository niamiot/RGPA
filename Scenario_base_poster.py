...
#		                     
#				self.std_v=np.array([[  4.34000000e+00,   6.66666667e-09,   6.66666667e-09]])
#				self.bn=np.array([[ 12.59218704,  16.84867628]])
...
#        file1=open('Geo_vs_ML.txt', 'w')
		...
		BS1 = np.array([2*L/3.0,0.0,H1])
                ...
                an = vstack((an,BS3))
                an = vstack((an,BS4))
#		BS1 = np.array([0.,70.,H1])
#                l_connect.append('RSS')
#                BS2 = np.array([50.,20.,H3])
#                l_connect.append('RSS')
##                BS3 = np.array([0.0,L/3.0,H1])
##                l_connect.append('RSS')
#                an = vstack((an,BS1))
#                an = vstack((an,BS2))
##                an = vstack((an,BS3))
...
# 		MS1 = np.array([20.,200.,H1])
#                l_connect.append('TOA')
#                MS2 = np.array([0.,14.,H2])
#                l_connect.append('TOA')
#                MS3 = np.array([2*L/3.0,L,H2])
#                l_connect.append('TOA')
##                MS4 = np.array([0.0,2*L/3.0,H3])
...
#                filename = filename + '_TDOA'
	...
	if ndim == 3:
        ...
        parmsc['exclude_out']    = None#np.array((BOUND1 ,BOUND2)) # contraint boundary or None
        parmsc['vcw']            = 1.0
        parmsc['save_pe']        = True 
        parmsc['save_CLA']       = False 

        parmsc['sigma_max_RSS']      = 4.34        # en (dB) 
        parmsc['sigma_max_TOA']      = 2.97       # en (m)  
        parmsc['sigma_max_TDOA']      = 3.55      # en (m)  
        ...
        parmsh = {}
        parmsh['display']=True     # launch geomview interactively 
